...
LLMとのチャットアプリのようなレイアウトを作成してください。人間のメッセージは右寄せ、AIの応答は左寄せに配置します。
...
**期待される結果:**
- チャットコンテナが縦方向のフレックスコンテナになっている
- AIのメッセージが左寄せで表示されている
- 人間のメッセージが右寄せで表示されている
- メッセージが縦方向にリスト形式で配置されている
