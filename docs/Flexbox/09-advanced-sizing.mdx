...
高度なサイズ制御を理解することで、より柔軟で予測可能なFlexboxレイアウトを作成できます。

### 重要なポイント

- **width vs flex-basis**: `flex-basis` はFlexboxの計算における理想的なサイズ、`width` は固定幅
- **制約の適用**: `min-width` と `max-width` はFlex計算結果を制限する
- **計算順序**: 初期サイズ → flex-grow/shrink → 制約適用の順番で処理される
- **margin: auto**: 特殊な動作については[専用ページ](./10-margin-auto)で詳しく解説

これらの概念を組み合わせることで、複雑なレイアウト要件にも対応できるようになります。